...
pub fn render_main_readme(
    ...
    let top_dirs: Vec<DirectoryRef> = structure.directories.iter()
        .map(|dir| DirectoryRef {
            name: dir.name.clone(),
            display_name: dir.display_name.clone().unwrap_or_else(|| dir.name.clone()),
            description: dir.description.clone(),
            access_level: dir.access_level.clone().unwrap_or_else(|| "team".to_string()),
    ...
    let frontmatter = RootFrontmatter {
        ...
        governance: structure.metadata.as_ref().and_then(|m| m.governance.as_ref().map(|g| GovernanceInfo {
            owner: g.owner.clone(),
    ...
    writeln!(content, "This directory structure was generated on {} using the SaaS Share Drive Bootstrapper tool.\n", current_date)?;
    ...
    // Add footer
    writeln!(content, "\n---\n")?;
    writeln!(content, "*This directory structure is maintained according to the organization's data governance policies.*")?;
...
pub fn render_directory_readme(
    ...
    let frontmatter = DirectoryFrontmatter {
        ...
        display_name: directory.display_name.clone().unwrap_or_else(|| directory.name.clone()),
        description: directory.description.clone(),
        path: path.to_string(),
        parent_structure: structure.name.clone(),
        organization: org_name.to_string(),
        access_level: directory.access_level.clone().unwrap_or_else(|| "team".to_string()),
    ...
    writeln!(content, "**Access Level:** {}  ", access_level_badge(access_level))?;
