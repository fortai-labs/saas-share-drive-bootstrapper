...
pub fn validate_schema_file(schema_path: &Path) -> Result<()> {
    let schema_content = fs::read_to_string(schema_path).map_err(AppError::Io)?;

    let schema_value: Value = serde_json::from_str(&schema_content).map_err(AppError::Json)?;
...
fn validate_directory_structure(schema: &Value) -> Result<()> {
    ...
    let obj = schema
        .as_object()
        .ok_or_else(|| AppError::InvalidSchema("Schema must be a JSON object".to_string()))?;
    ...
    if let Some(directories) = obj.get("directories") {
        if !directories.is_array() {
            return Err(AppError::InvalidSchema(
                "'directories' must be an array".to_string(),
            ));
        }
        ...
        if directories_arr.is_empty() {
            return Err(AppError::InvalidSchema(
                "'directories' array cannot be empty".to_string(),
            ));
        }
...
fn validate_directory(dir: &Value, index: usize) -> Result<()> {
    let obj = dir.as_object().ok_or_else(|| {
        AppError::InvalidSchema(format!(
            "Directory at index {} must be a JSON object",
            index
        ))
    })?;
    ...
    for field in &["name", "description"] {
        if !obj.contains_key(*field) {
            return Err(AppError::MissingField(format!(
                "Directory at index {} is missing required field: {}",
                index, field
            )));
        }
    }
    ...
    if let Some(subdirs) = obj.get("subdirectories") {
        if !subdirs.is_array() {
            return Err(AppError::InvalidSchema(format!(
                "'subdirectories' in directory at index {} must be an array",
                index
            )));
        }
