...
fn test_render_main_readme_from_example() {
    ...
    let expected_content =
        fs::read_to_string(expected_readme_path).expect("Failed to read expected README");
    ...
    assert!(
        rendered_content.contains("# SaaS_Company_Drive"),
        "README should contain the structure name"
    );
    assert!(
        rendered_content.contains("Centralized repository"),
    ...
    for dir in &structure.directories {
        assert!(
            rendered_content.contains(&dir.name),
            "README should mention directory {}",
            dir.name
        );
    }
}
...
fn test_render_directory_readme_from_example() {
    ...
    assert!(
        result.is_ok(),
        "Failed to render Engineering README: {:?}",
        result.err()
    );
    ...
    // Check for key elements in the rendered content
    assert!(
        rendered_content.contains("# Engineering"),
        "README should contain the directory name"
    );
    assert!(
        rendered_content.contains("Engineering resources"),
    ...
    if let Some(subdirs) = &engineering_dir.subdirectories {
        for subdir in subdirs {
            assert!(
                rendered_content.contains(&subdir.name),
                "README should mention subdirectory {}",
                subdir.name
            );
        }
    }
...
fn test_render_subdirectory_readme_from_example() {
    ...
    let api_docs_dir = engineering_dir
        .subdirectories
        .as_ref()
    ...
    assert!(
        result.is_ok(),
        "Failed to render API_Documentation README: {:?}",
        result.err()
    );
    ...
    // Read the expected README content
    let expected_readme_path =
        Path::new("examples/generic-saas/output/03_Engineering/API_Documentation/README.md");
    let expected_content =
        fs::read_to_string(expected_readme_path).expect("Failed to read expected README");
    ...
    // Check for key elements in the rendered content
    assert!(
        rendered_content.contains("# API Documentation"),
        "README should contain the directory name"
    );
    assert!(
        rendered_content.contains("API specifications"),
        "README should contain the directory description"
    );
