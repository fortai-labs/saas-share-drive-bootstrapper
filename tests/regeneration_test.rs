...
fn compare_directories(dir1: &Path, dir2: &Path) -> Result<(), String> {
    ...
    for file in &files1 {
        if !files2.contains(file) {
            return Err(format!(
                "File {:?} exists in original but not in regenerated",
                file
            ));
        }
    }
    ...
    for file in &files2 {
        if !files1.contains(file) {
            return Err(format!(
                "File {:?} exists in regenerated but not in original",
                file
            ));
        }
    }
...
fn test_regenerate_output() {
    ...
    let result =
        generator::generate_structure(&structure, regenerated_output_dir, Some("SaaS Company"));
    assert!(
        result.is_ok(),
...
fn test_regenerate_subdirectory() {
    ...
    assert!(
        result.is_ok(),
        "Failed to regenerate Engineering directory: {:?}",
        result.err()
    );
    ...
    assert!(
        eng_dir_path.exists() && eng_dir_path.is_dir(),
        "Engineering directory not created"
    );
    ...
    assert!(
        readme_path.exists(),
        "README.md not found in Engineering directory"
    );
